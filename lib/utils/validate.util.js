...
var Validator = /** @class */ (function () {
    ...
    Validator.prototype.loadRules = function (rules) {
        ...
        return lodash_1.default.reduce(rules, function (result, options, fieldKey) {
            var _a;
            var loadedRule = _this.loadRule(fieldKey, options);
            var storedRules = (_a = Reflect.get(result, fieldKey)) !== null && _a !== void 0 ? _a : [];
            storedRules.push.apply(storedRules, loadedRule);
            Reflect.set(result, fieldKey, storedRules);
            return result;
        }, {});
    };
    Validator.prototype.loadRule = function (fieldKey, options) {
        if (options === void 0) { options = {}; }
        var i18n = this.getI18n();
        var label = i18n.t(fieldKey);
        if (Array.isArray(options)) {
            return options.map(function (option) {
                return validateUtil.getRule((0, tslib_1.__assign)({ label: label }, option));
            });
        }
        return [validateUtil.getRule((0, tslib_1.__assign)({ label: label }, options))];
    };
    return Validator;
}());
