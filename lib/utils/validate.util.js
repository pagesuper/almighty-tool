...
var lodash_es_1 = tslib_1.__importDefault(require("lodash-es"));
...
var validateUtil = {
    ...
    getErrors: function (error, options) {
        ...
        if (typeof error === 'object' && error !== null && 'errors' in error) {
            return Reflect.get(error, 'errors').map(function (err) {
                var field = lodash_es_1.default.get(err, 'field');
                var fieldValue = field ? lodash_es_1.default.get(values, field) : undefined;
                return {
                    field: field,
        ...
        return [
            {
                ...
                fieldValue: (_c = options === null || options === void 0 ? void 0 : options.fieldValue) !== null && _c !== void 0 ? _c : ((options === null || options === void 0 ? void 0 : options.field) ? lodash_es_1.default.get(values, options.field) : undefined),
                model: model,
            },
        ];
    },
    ...
    transform: function (values, rules) {
        ...
        function doTransform(values, parentPath) {
            ...
            return lodash_es_1.default.transform(values, function (result, value, key) {
            ...
            return tslib_1.__generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        ...
                        usingValues = lodash_es_1.default.omit((options === null || options === void 0 ? void 0 : options.pickKeys) ? lodash_es_1.default.pick(values, options.pickKeys) : values, (_b = options === null || options === void 0 ? void 0 : options.omitKeys) !== null && _b !== void 0 ? _b : []);
    ...
    recursiveGetLocaleRules: function (rules, flatRules, options) {
        ...
        Object.keys(rules).forEach(function (fieldKey) {
            ...
            var defaultFields = lodash_es_1.default.reduce(fieldRules, function (result, rule) {
            ...
            fieldRules.forEach(function (rule) {
                ...
                if (rule.path) {
                    var flatRule = lodash_es_1.default.cloneDeep(rule);
    ...
    parseRules: function (rules, initialRules, options) {
        ...
        var mergedRules = lodash_es_1.default.reduce(rules, function (result, rule, fieldKey) {
            var _a, _b;
            var loadedRules = [];
            ...
            return result;
        }, lodash_es_1.default.isEmpty(initialRules) ? {} : validateUtil.parseRules(initialRules, {}));
    ...
    parseToRules: function (opts) {
        var rules = [];
        var options = lodash_es_1.default.cloneDeep(opts);
        if (options.message) {
            ...
            if (isPresent(options.regexpKey) || isPresent(options.pattern) || isPresent(options.asyncValidator)) {
                var ruleRegexpKey = lodash_es_1.default.cloneDeep(options);
            ...
            if (isPresent(options.enum)) {
                var ruleEnum = lodash_es_1.default.cloneDeep(options);
            ...
            if (isPresent(options.len)) {
                var ruleLen = lodash_es_1.default.cloneDeep(options);
            ...
            if (isPresent(options.min) && isPresent(options.max)) {
                var ruleMinMax = lodash_es_1.default.cloneDeep(options);
            ...
            if (isPresent(options.min)) {
                var ruleMin = lodash_es_1.default.cloneDeep(options);
            ...
            if (isPresent(options.max)) {
                var ruleMax = lodash_es_1.default.cloneDeep(options);
            ...
            if (isPresent(options.pattern)) {
                var rulePattern = lodash_es_1.default.cloneDeep(options);
            ...
            if (isPresent(options.whitespace)) {
                var ruleWhitespace = lodash_es_1.default.cloneDeep(options);
            ...
            if (options.defaultField || options.fields) {
                var ruleFields = lodash_es_1.default.cloneDeep(options);
            ...
            if (options.required) {
                var ruleRequired = lodash_es_1.default.cloneDeep(options);
                ...
                if (!lodash_es_1.default.isEmpty(lodash_es_1.default.pick(lodash_es_1.default.omit(options, omitKeys), avaliableKeys))) {
                    rules.push(validateUtil.parseRule(options));
                }
            }
            if (isPresent(options.transformers) || typeof options.transform === 'function') {
                var ruleTransformers = lodash_es_1.default.cloneDeep(options);
    ...
    parseRule: function (options, parseRuleOptions) {
        ...
        var message = (function () {
            var _a;
            var pickedRules = lodash_es_1.default.pick(options, SIMPLE_RULE_KEYS);
        ...
        if (parseRuleOptions.parseAsyncValidator) {
            var asyncValidator = (function () {
                ...
                var regexp = (function () {
                    ...
                    return regexpKey ? lodash_es_1.default.get(format_util_1.regExps, regexpKey) : undefined;
                })();
        ...
        if (parseRuleOptions.parseSubFields) {
                ...
                if (defaultFieldType === 'object') {
                    defaultFields.forEach(function (defaultField) {
                        ...
                        if (defaultField.fields) {
                            defaultField.fields = lodash_es_1.default.reduce(defaultField.fields, function (result, field, fieldKey) {
                                var fields = [];
                    ...
                    rule.defaultField = lodash_es_1.default.reduce(defaultFields, function (result, defaultField) {
            ...
            if (options.fields) {
                rule.fields = lodash_es_1.default.reduce(options.fields, function (result, field, fieldKey) {
                    var fields = [];
                    (Array.isArray(field) ? field : [field]).forEach(function (field) {
                        fields.push.apply(fields, validateUtil.parseToRules(tslib_1.__assign({ path: "".concat(path, ".").concat(fieldKey) }, field)));
                    });
    ...
    collectRulesRequiredAssign: function (requires, rules) {
        Object.keys(rules).forEach(function (fieldKey) {
            ...
            fieldRules.forEach(function (rule, ruleIndex) {
                var _a;
                if (ruleIndex === 0 && rule.path && lodash_es_1.default.last((_a = requires[rule.path]) !== null && _a !== void 0 ? _a : [])) {
                    rule.required = true;
    ...
    normalizeRules: function (rules, options) {
        ...
        var pureRules = lodash_es_1.default.cloneDeep(rules);
...
var Validator = /** @class */ (function () {
    ...
    Validator.prototype.wrapRules = function (options) {
        ...
        var validator = override ? this : lodash_es_1.default.cloneDeep(this);
    ...
    Validator.prototype.omitRules = function (options) {
        ...
        var override = (_a = options.override) !== null && _a !== void 0 ? _a : false;
        var validator = override ? this : lodash_es_1.default.cloneDeep(this);
        validator.rules = lodash_es_1.default.omit(validator.rules, options.fieldKeys);
        return validator;
    };
    return Validator;
}());
