...
function getCharsByRanges(ranges) {
    ...
    if (typeof ranges === 'string') {
        ...
        return lodash_es_1.default.uniq(lodash_es_1.default.reduce(ranges, function (result, rangeKey) {
            var _a;
...
var randomUtil = {
    ...
    generateRandomString: function (options) {
        ...
        for (var index = 0; index < randomLength; index++) {
            var sample = lodash_es_1.default.sample(characters);
            if (sample) {
                values.push(sample);
            }
        }
    ...
    getUtcTimeString: function (dateTime) {
        ...
        return [
            time.getUTCFullYear(),
            lodash_es_1.default.padStart(String(time.getUTCMonth() + 1), 2, '0'),
            lodash_es_1.default.padStart(String(time.getUTCDate()), 2, '0'),
            lodash_es_1.default.padStart(String(time.getUTCHours()), 2, '0'),
            lodash_es_1.default.padStart(String(time.getUTCMinutes()), 2, '0'),
            lodash_es_1.default.padStart(String(time.getUTCSeconds()), 2, '0'),
            lodash_es_1.default.padStart(String(time.getUTCMilliseconds()), 3, '0'),
        ].join('');
    },
};
