...
function getCharsByRanges(ranges) {
    ...
    if (typeof ranges === 'string') {
        ...
        return (0, lodash_es_1.uniq)((0, lodash_es_1.reduce)(ranges, function (result, rangeKey) {
            var _a;
...
var randomUtil = {
    ...
    generateRandomString: function (options) {
        ...
        for (var index = 0; index < randomLength; index++) {
            var sample = (0, lodash_es_1.sample)(characters);
            if (sample) {
                values.push(sample);
            }
        }
    ...
    getUtcTimeString: function (dateTime) {
        ...
        return [
            time.getUTCFullYear(),
            (0, lodash_es_1.padStart)(String(time.getUTCMonth() + 1), 2, '0'),
            (0, lodash_es_1.padStart)(String(time.getUTCDate()), 2, '0'),
            (0, lodash_es_1.padStart)(String(time.getUTCHours()), 2, '0'),
            (0, lodash_es_1.padStart)(String(time.getUTCMinutes()), 2, '0'),
            (0, lodash_es_1.padStart)(String(time.getUTCSeconds()), 2, '0'),
            (0, lodash_es_1.padStart)(String(time.getUTCMilliseconds()), 3, '0'),
        ].join('');
    },
};
