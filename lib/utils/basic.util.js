...
var each_1 = tslib_1.__importDefault(require("lodash-es/each"));
...
var basicUtil = {
    ...
    cssObjectToString: function (style) {
        var styles = [];
        (0, each_1.default)(style, function (value, key) {
    ...
    retry: function (fn_1) {
        return tslib_1.__awaiter(this, arguments, void 0, function (fn, retries, delay) {
            var err_1;
            ...
            return tslib_1.__generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 3:
                        _a.sent();
                        return [2 /*return*/, this.retry(fn, retries - 1, delay)];
                    case 4: return [2 /*return*/];
                }
            });
        });
     ...
     */
    pickFieldsLikeGraphQL: function (data, fields) {
        var _this = this;
        // 无筛选 / 空数据 → 直接返回
        if (!fields || !data)
            return data;
        ...
        if (Array.isArray(data)) {
            return data.map(function (item) { return _this.pickFieldsLikeGraphQL(item, fields); });
        }
        var result = {};
        // 1. 清洗：去掉所有空白字符（空格、换行、tab）
        var cleanFields = fields.replace(/\s+/g, '');
        // 2. 递归解析函数（真正支持无限层级嵌套）
        var parseFields = function (str, target, source) {
            var i = 0;
            var len = str.length;
            while (i < len) {
                // 读取字段名
                ...
                if (i < len && str[i] === '{') {
                    ...
                    var innerStr = str.slice(start, i - 1);
                    target[key] = {};
    ...
    // =========================================================================
    cleanEmptyValues: function (obj, options) {
        var _this = this;
        if (options === void 0) { options = {}; }
        var opt = tslib_1.__assign({ cleanNull: true, cleanUndefined: true, cleanEmptyString: true, cleanFalse: true, cleanZero: false, cleanEmptyObject: true, cleanEmptyArray: true }, options);
        // 基础类型
        ...
        if (Array.isArray(obj)) {
            var processed = obj
                .map(function (item) { return _this.cleanEmptyValues(item, opt); })
                .filter(function (item) {
                if (opt.cleanNull && item === null)
                    return false;
                if (opt.cleanUndefined && item === undefined)
                    return false;
                if (opt.cleanEmptyString && item === '')
                    return false;
                if (opt.cleanFalse && item === false)
                    return false;
                if (opt.cleanZero && item === 0)
                    return false;
                if (opt.cleanEmptyArray && Array.isArray(item) && item.length === 0)
                    return false;
                if (opt.cleanEmptyObject && isPlainObject(item) && Object.keys(item).length === 0)
                    return false;
                return true;
            });
            return processed;
        ...
        for (var _i = 0, _a = Object.entries(obj); _i < _a.length; _i++) {
            ...
            if (opt.cleanNull && value === null)
                continue;
            if (opt.cleanUndefined && value === undefined)
                continue;
            if (opt.cleanEmptyString && value === '')
                continue;
            if (opt.cleanFalse && value === false)
                continue;
            if (opt.cleanZero && value === 0)
                continue;
            cleaned[key] = this.cleanEmptyValues(value, opt);
        }
        // 二次过滤空对象/空数组
        var result = {};
        for (var _c = 0, _d = Object.entries(cleaned); _c < _d.length; _c++) {
            var _e = _d[_c], key = _e[0], val = _e[1];
            if (opt.cleanEmptyObject && isPlainObject(val) && Object.keys(val).length === 0)
                continue;
            if (opt.cleanEmptyArray && Array.isArray(val) && val.length === 0)
                continue;
            result[key] = val;
        }
        return result;
