...
const languageLangCodeMap = Object.values(languageLocaleMap).reduce((acc, item) => {
  ...
  detectAll: (text: string, options?: Partial<DetectOption>) => {
    ...
    if (options?.only) {
      ...
      for (const item of options.only) {
        const language = Reflect.get(languageCodeMap, item);
    ...
    return result.map((item) => {
      return {
        ...item,
        key: Reflect.get(languageLangCodeMap, item.lang) as string,
      };
    });
  },

  getKeyByText: (text: string, options?: Partial<DetectOption>) => {
    const result = languageUtil.detectAll(text, options);
