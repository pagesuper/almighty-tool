...
const DEFAULT_I18N_KEY = 'tuitui-lib/formats/date-format#i18n';
      ...
      switch (options.formatter) {
        ...
        case 'shortStep':
          if (now.startOf('day').diff(day) <= 0 && now.add(1, 'day').startOf('day').diff(day) > 0) {
            /** 今天 */
            return day.locale(locale).format(_i18n.t(`KinlongLib.DateFormat.${options.formatter}.today`).toString());
          } else if (now.add(1, 'day').startOf('day').diff(day) <= 0 && now.add(2, 'day').startOf('day').diff(day) > 0) {
            /** 明天 */
            return day.locale(locale).format(_i18n.t(`KinlongLib.DateFormat.${options.formatter}.tomorrow`).toString());
          } else if (now.subtract(1, 'day').startOf('day').diff(day) <= 0 && now.startOf('day').diff(day) > 0) {
            /** 昨天 */
            return day.locale(locale).format(_i18n.t(`KinlongLib.DateFormat.${options.formatter}.yesterday`).toString());
          } else if (now.startOf('year').diff(day) <= 0 && now.add(1, 'year').startOf('year').diff(day) > 0) {
            /** 今年 */
            return day.locale(locale).format(_i18n.t(`KinlongLib.DateFormat.${options.formatter}.thisYear`).toString());
          }

          return day.locale(locale).format(_i18n.t(`KinlongLib.DateFormat.${options.formatter}.longAgo`).toString());
